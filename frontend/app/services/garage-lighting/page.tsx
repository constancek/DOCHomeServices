...
export default function GarageLightingPage() {
  return (
    <>
      ...
      {/* Garage lighting by neighborhood — links DOWN to location pages */}
        ...
        <div className="container-page max-w-4xl">
          ...
          <div className="mt-6 space-y-7">
            {locationsByGroup().map(({ group, items }) => {
              const local = items.filter((l) => garageLightingCopy[l.slug]);
                  ...
                  <ul className="mt-2 columns-1 gap-x-6 sm:columns-2">
                    {local.map((l) => (
                      <li key={l.slug} className="mb-1.5 break-inside-avoid">
                        <Link
                          href={`/services/garage-lighting/${l.slug}`}
                          className="text-sm font-medium text-brand-800 hover:text-pink-600"
                        >
                          Garage lighting in {l.neighborhood}
                        </Link>
                      </li>
                    ))}
                  </ul>
